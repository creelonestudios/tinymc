...
export default class WorldGenerator {
	...
	static flat(world: World) {
		for (let y = world.minY; y <= world.maxY && y <= 0; y++) {
			for (let x = world.minX; x <= world.maxX; x++) {
				...
				for (let z = world.minZ; z <= 0; z++) world.setBlock(x, y, z, new Block(blockId))
			}
		}

		for (let x = world.minX; x <= world.maxX; x++) for (let z = world.minZ; z <= world.maxZ; z++) world.scheduleBlockUpdate(x, world.maxY, z)
	...
	static random(world: World) {
		const blockArray = Array.from(blockdefs.values())
