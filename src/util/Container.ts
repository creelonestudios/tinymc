...
export default class Container {
	...
	static getMouseSlot() {
		...
		if (slotIndex >= 0 && slotIndex < inventory.size) return { slotPos, slotIndex }
	...
	static onClick(button: number): boolean { // true -> an action was performed
		...
		if (button == 0) {
			if (input.keyPressed("ShiftLeft")) { // move stack to hotbar
				...
				if (leftOver) stack.amount = leftOver.amount
				 else inventory.set(mouseSlot.slotIndex, new ItemStack("tiny:air"))
			} else {
				const stack = inventory.get(mouseSlot.slotIndex)
...
function drawSlots(g: Graphics, _inventory: Inventory, cb: (inv: Inventory, i: number) => void) {
	const { ctx } = g

	ctx.save()

	let column = 0
	for (let i = 0; i < _inventory.size; i++) {
		cb(_inventory, i)

		// translate
		column++
		if (column < _inventory.columns) ctx.translate(slotSize, 0)
		 else {
			ctx.translate(-(_inventory.columns - 1) * slotSize, slotSize)
			column = 0
		}
	}
